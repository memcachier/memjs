...
function makeClient(dummyServer: MemJS.Server | MemJS.Server[], options?: GivenClientOptions<MaybeBuffer, any>) {
...
test("GetMultiError_MultiBackend", function (t) {
	...
	dummyServer2.write = function () {
		dummyServer2.error({ name: "ErrorName", message: "This is an expected error." })
	}
	const servers = [dummyServer1, dummyServer2]
...
test("GetMultiError", function (t) {
	...
	dummyServer.write = function (requestBuf) {
		...
		checkAndRespond(requests[0], "hello1", "world1")
		dummyServer.error({ name: "ErrorName", message: "This is an expected error." })
	}
