...
exports.makeRequestBuffer = function(opcode, key, extras, value, opaque, serializer) {
  serializer = serializer || nonSerialization;
  key = bufferify(key);
  extras = bufferify(extras);
  if(value) {
    extras = serializer.storeInfoByValueInExtras(value, extras);
  }
  value = bufferify(serializer.serializeValue(value));
...
exports.parseMessage = function(dataBuf, serializer) {
  serializer = serializer || nonSerialization;
  ...
  var data_type = serializer.getTypeInfoFromExtra(extras);

  return {header: responseHeader, key: key, extras: extras, val: serializer.deSerializeValue(val, data_type)};
};
