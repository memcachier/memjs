...
var Server = function(host, port, username, password, options) {
  ...
  this.errorCallbacks = {};
  this.options = merge(options || {}, {timeout: 0.5, keepAlive: false, keepAliveDelay: 30, serializer: nonSerialization});
...
Server.prototype.listSasl = function() {
  var buf = makeRequestBuffer(0x20, '', '', '', this.serializer);
  this.writeSASL(buf);
};

Server.prototype.saslAuth = function() {
  var authStr = '\x00' + this.username + '\x00' + this.password;
  var buf = makeRequestBuffer(0x21, 'PLAIN', '', authStr, this.serializer);
  this.writeSASL(buf);
};
...
Server.prototype.responseHandler = function(dataBuf) {
  var response = parseMessage(this.appendToBuffer(dataBuf), this.serializer);
  var respLength;
  while (response) {
    ...
    response = parseMessage(this.responseBuffer, this.serializer);
  }
};
