...
// INCREMENT
//
// Takes a key to increment from memcache and a callback. An error is passed as
// the first argument to the callback and the result of the
// operation is signaled through the argument to the callback.
Client.prototype.increment = function(key, amount, valueIfEmpty, callback) {
  var extras = makeAmountInitialAndExpiration(amount,valueIfEmpty,this.options.expires);
  var request = makeRequestBuffer(5, key, extras, '');
  ...
  this.perform(serv, request, function(response) {
    switch (response.header.status) {
    case  0:
      callback && callback(null, new Buffer(response.val).readUInt32BE(4), response.extras)
      break;
    case 1:
      callback && callback(null, null, null);
      break;
    default:
      var errorMessage = 'MemJS INCREMENT: ' + response.header.status;
      logger.log(errorMessage);
      callback && callback(new Error(errorMessage), null, null);
    }
  });
...
// the first argument to the callback and the result of the
// operation is signaled through the argument to the callback.
Client.prototype.decrement = function(key, amount, valueIfEmpty, callback) {
  var extras = makeAmountInitialAndExpiration(amount,valueIfEmpty,this.options.expires);
  var request = makeRequestBuffer(6, key, extras, '');
  var serv = this.server(key);
  var logger = this.options.logger;
  this.perform(serv, request, function(response) {
    switch (response.header.status) {
...
Client.prototype.flush = function(callback) {
  var request = makeRequestBuffer(0x08, '', '', '');
  var result = true;
...
Client.prototype.perform = function(serv, request, callback, retries) {
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      serv.write(request, responseHandler);
    } else {
