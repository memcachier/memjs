...
Client.prototype._getMultiToServer = function (serv, keys, callback) {
  ...
  var handle = (response) => {
    ...
    case constants.RESPONSE_STATUS_KEY_NOT_FOUND:
      ...
      break;
    default:
      var errorMessage = 'MemJS GET-MULTI: ' + errors[response.header.status];
  ...
  // this.performOnServer(serv, request, this.seq, handle, /*retries*/ undefined, /* afterFailover */ undefined);
};
  ...
  const errorHandler = (error) => {
    if (--retries > 0) {
      ...
      if (failover && afterFailover) {
        ...
        failover(server.hostportString);
