  ...
  if(callback === undefined) {
    return promisify(function(callback) {
      self.getMulti(keys, function(err, values, flags) {
        callback(err, {value: values, flags: flags});
      });
    });
  }
...
Client.prototype._version = function(serverKey, callback) {
  var self = this;
  if(callback === undefined) {
    return promisify(function(callback) {
      self._version(serverKey, function(err, value, flags) {
        callback(err, {value: value, flags: flags});
      });
    });
  }
...
Client.prototype.version = function(callback) {
  return this._version(this.serverKeys[0], callback);
};

Client.prototype.versionAll = function(callback) {
  const promise = Promise.all(this.serverKeys.map((serverKey) => {
    return this._version(serverKey).then((response) => {
      return {serverKey: serverKey, value: response.value};
    });
  })).then(versionObjects => {
    const value = versionObjects.reduce((accumulator, versionObject) => {
