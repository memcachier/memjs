  ...
  if (this.options.failover === true) {
    ...
    this.options.failover = (serverKey) => {
      ...
      if (server) {
        ...
        throw new Error(`No server found on failover for serverKey: ${serverKey}`);
      }
    };
  }
...
Client.prototype.set = function(key, value, options, callback) {
  if(callback === undefined && typeof options !== 'function') {
    if (!options) options = {};
    return promisify((callback) => { this.set(key, value, options, function(err, success) { callback(err, success); }); });
  }
  var logger = this.options.logger;
  var expires;
...
Client.prototype.perform = function(key, request, seq, callback, retries) {
  const performCallback = () => {
    ...
    this.performOnServer(server, request, seq, callback, retries, /* onFailover: */ performCallback);
  };
...
Client.prototype.performOnServer = function(
  server, request, seq, callback, retries, onFailover) {
  ...
  const failover = this.options.failover;
  ...
  const errorHandler = (error) => {
    if (--retries > 0) {
      ...
      setTimeout(() => {
        this.performOnServer(server, request, seq, callback, retries);
      }, 1000 * retry_delay);
    } else {
      ...
      if (failover) {
        if (onFailover) {
          onFailover(server.hostportString());
        }
      } else {
...
Client.prototype.defaultKeyToServerHashFunction = function(servers, key) {
  const total = servers.length;
  const origIndex = (total > 1) ? (hashCode(key) % total) : 0;
  let server = this.servers[origIndex];
  let index = origIndex;
